...
//     "artwork": <base64 of the RAW artwork bytes, whatever format the
//     source app published (usually JPEG or PNG) — resizing/re-encoding
//     to a consistent format happens Swift-side, not here, see
//     NowPlaying.swift>, "playing": bool }
...
void adapter_get(void) {
    @autoreleasepool {
        ...
        if (info) {
            ...
            if (artwork.length > 0) out[@"artwork"] = [artwork base64EncodedStringWithOptions:0];
        }
        out[@"playing"] = @(playing);
